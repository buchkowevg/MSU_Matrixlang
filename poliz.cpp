    ...
    try
    {
        ...
        case LEX_INTEGER:
            ...
            else if(op2->get_type() == LEX_INTEGER)
            {
                ...
                pol.push(new Poliz_Const
                         (new Ident_Rational
                          ("", Rational_number(to_string(static_cast<Ident_Int*>(op1)->get_value()).c_str() ,
                                               to_string(static_cast<Ident_Int*>(op2)->get_value()).c_str())), Lex(), LEX_TEMP_CONST));
            }
            else if(op2->get_type() == LEX_MATRIX)
                pol.push(new Poliz_Const
                         (new Ident_Matrix
    ...
    try
    {
        ...
        case LEX_MATRIX:
            if(op2->get_type() == LEX_INTEGER)
                pol.push(new Poliz_Const
                         (new Ident_Vector
                          ("", static_cast<Ident_Matrix*>(op1)->get_value().get_row
                           (static_cast<Ident_Int*>(op2)->get_value())),
                          Lex(), LEX_TEMP_CONST));
            else if(op2->get_type() == LEX_RATIONAL)
                pol.push(new Poliz_Const
    ...
    try
    {
        ...
        case LEX_MATRIX:
            if(op2->get_type() == LEX_INTEGER)
                pol.push(new Poliz_Const
                         (new Ident_Vector
                          ("", static_cast<Ident_Matrix*>(op1)->get_value().get_column
                           (static_cast<Ident_Int*>(op2)->get_value())),
                          Lex(), LEX_TEMP_CONST));
            else if(op2->get_type() == LEX_RATIONAL)
                pol.push(new Poliz_Const
...
void Poliz_Function::make_canonical(std::stack<Poliz *> &pol)
{
    ...
    try
    {
        ...
        case LEX_VECTOR:
            ...
            break;
        case LEX_MATRIX:
            m = static_cast<Ident_Matrix*>(op)->get_value();
            m.make_canonical();
            pol.push(new Poliz_Const
                     (new Ident_Matrix
